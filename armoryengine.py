...
ARMORY_DONATION_ADDR = '14mLVV4WoM8xsjtjavbRogQK6Haxzz3xA6'
...
parser.add_option("--satoshi-datadir", dest="satoshiHome", default='DEFAULT', type='str', 
                  help="The Terracoin-Qt/terracoind home directory")
parser.add_option("--satoshi-port", dest="satoshiPort", default='DEFAULT', type="str",
                  help="For Terracoin-Qt instances operating on a non-standard port")
...
# Allow user to override default terracoin-qt/terracoind home directory
...
def findLatestBlkFiles(baseDir):
   # Terracoin-Qt 0.8+ uses new location.  User might also be specifying
...
print '   Satoshi TRC directory :', BTC_HOME_DIR
...
if not USE_TESTNET:
   ...
   BITCOIN_PORT = 18332
   MAGIC_BYTES = '\x42\xba\xbe\x56'
   GENESIS_BLOCK_HASH_HEX  = 'a2af77a2261e246b8a0fd7f2cce192f469e44c56bb9d1a626abc4b8000000000'
   GENESIS_BLOCK_HASH      = '\xa2\xafw\xa2&\x1e$k\x8a\x0f\xd7\xf2\xcc\xe1\x92\xf4i\xe4LV\xbb\x9d\x1abj\xbcK\x80\x00\x00\x00\x00'
   GENESIS_TX_HASH_HEX     = '96f12836f9a4d8029fea2c89ad06be01a9aaa6f3c3160c5867b00338f9098b0f'
   GENESIS_TX_HASH         = '\x96\xf1(6\xf9\xa4\xd8\x02\x9f\xea,\x89\xad\x06\xbe\x01\xa9\xaa\xa6\xf3\xc3\x16\x0cXg\xb0\x038\xf9\t\x8b\x0f'
   ADDRBYTE = '\x00'
   PRIVKEYBYTE = '\x80'
else:
   BITCOIN_PORT = 18333
...
if not CLI_OPTIONS.satoshiPort == 'DEFAULT':
   try:
      ...
      raise TypeError, 'Invalid port for Terracoin-Qt, using ' + str(BITCOIN_PORT)
...
LOGINFO('   Satoshi TRC directory : ' + BTC_HOME_DIR)
...
def coin2str(nSatoshi, ndec=8, rJust=True, maxZeros=8):
   """
   Converts a raw value (1e-8 TRC) into a formatted string for display
...
# operations we ever do in the terracoin network
...
def binary_to_base58(binstr):
   """
   This method applies the Terracoin-specific conversion from binary to Base58
...
def base58_to_binary(addr):
   """
   This method applies the Terracoin-specific conversion from Base58 to binary
   which may includes some extra "zero" bytes, such as is the case with the
   main-network addresses.
...
def hash160_to_addrStr(binStr):
   """
   Converts the 20-byte pubKeyHash to 25-byte binary Terracoin address
...
# Taken directly from rpc.cpp in reference terracoin client, 0.3.24
...
# The following params are for the Terracoin elliptic curves (secp256k1)
...
# Check validity of a TRC address in its binary form, as would
...
# Check validity of a TRC address in Base58 form
...
class PyBtcAddress(object):
   ...
   def pprint(self, withPrivKey=True, indent=''):
      ...
      print indent + 'TRC Address      :', self.getAddrStr()
...
class PyScriptProcessor(object):
   ...
   def checkSig(self,binSig, binPubKey, txOutScript, txInTx, txInIndex, lastOpCodeSep=None):
      """
      Generic method for checking Terracoin tx signatures.  This needs to be used for both
   ...
   def executeOpCode(self, opcode, scriptUnpacker, stack, stackAlt):
      ...
      # TODO: figure out the conditional op codes...
         ...
         # Location:  terracoin-0.4.0-linux/src/src/script.cpp:775
         i=1
         if len(stack) < i:
            return TX_INVALID
...
def pprintUnspentTxOutList(utxoList, headerLine='Coin Selection: '):
   ...
   print headerLine, '(Total = %s TRC)' % coin2str(totalSum)
   ...
   for utxo in utxoList:
      ...
      print '   ',(coin2str(utxo.getValue()) + ' TRC').rjust(18),
...
class PyTxDistProposal(object):
   ...
   def pprint(self, indent='   '):
      ...
      for i,txout in enumerate(tx.outputs):
         print '   Recipient: %s TRC' % coin2str(txout.value),
         scrType = getTxOutScriptType(txout.binScript)
         if scrType in (TXOUT_SCRIPT_STANDARD, TXOUT_SCRIPT_COINBASE):
...
class PyBtcWallet(object):
   ...
   ***NOTE:  I have ONLY implemented deterministic wallets, using ECDSA
   ...
   (3) May be watching *other* people's addrs.  There's a variety of reasons
       ...
       relevant to a "basic" TRC user.  Nonetheless it should be supported to
   ...
   magic bytes -- (4)   defines the blockchain for this wallet (TRC, NMC)
...
class PyLedgerEntry(object):
   ...
   //    value_     -  total debit/credit on WALLET balance, in Satoshis (1e-8 TRC)
...
class PyMessage(object):
   ...
   def pprint(self, nIndent=0):
      indstr = indent*nIndent
      print ''
      print indstr + 'Terracoin-Network-Message -- ' + self.cmd.upper()
...
#    part of the Terracoin protocol.  I need just enough to broadcast tx
...
class ArmoryClient(Protocol):
   """
   This is where all the Terracoin-specific networking stuff goes.
...
#      # ... and the Terracoin client is coded to understand:
...
#
#   if r is not None:
#      LOGERROR("Couldn't open wallet.dat/main. Try quitting Terracoin and running this again.")
#      sys.exit(1)
#   
#   return db
#
...
class BlockDataManagerThread(threading.Thread):
   ...
   def __startLoadBlockchain(self):
      ...
      # Check for the existence of the Terracoin-Qt directory
