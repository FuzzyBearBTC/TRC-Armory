...
class ArmoryMainWindow(QMainWindow):
   ...
   def __init__(self, parent=None):
      ...
      self.lblBTC1 = QRichLabel('<b>TRC</b>', doWrap=False)
      self.lblBTC2 = QRichLabel('<b>TRC</b>', doWrap=False)
      self.lblBTC3 = QRichLabel('<b>TRC</b>', doWrap=False)
      ...
      btnSendBtc   = QPushButton("Send Terracoins")
      btnRecvBtc   = QPushButton("Receive Terracoins")
      ...
      #actMigrateSatoshi = self.createAction('&Migrate Terracoin Wallet',    self.execMigrateSatoshi)
   ...
   def setupSystemTray(self):
      ...
      actSendBtc    = self.createAction('Send Terracoins', traySend)
      actRcvBtc     = self.createAction('Receive Terracoins', trayRecv)
            ...
            try:
               if self.connectCount>0:
                  self.sysTray.showMessage('Connected', \
                     'Connection to Terracoin-Qt re-established', \
   ...
   def confirmSweepScan(self, pybtcaddrList, targAddr160):
      ...
      if TheBDM.getBDMState() in ('Offline', 'Uninitialized'):
         ...
         msgConfirm = ( \
            ...
            'find any terracoins associated with the %s you supplied. '
   ...
   def broadcastTransaction(self, pytx, dryRun=False):
      ...
      if dryRun:
         ...
         def checkForTxInBDM():
            ...
            if not TheBDM.getTxByHash(newTxHash).isInitialized():
               ...
               QMessageBox.warning(self, 'Invalid Transaction', \
                  ...
                  'appear to have been accepted by the Terracoin network. '
         ...
         #QMessageBox.information(self, 'Broadcast Complete!', \
            #'The transaction has been broadcast to the Terracoin network.  However '
   ...
   def execMigrateSatoshi(self):
      reply = MsgBoxCustom(MSGBOX.Question, 'Wallet Version Warning', \
           'This wallet migration tool only works with regular Terracoin wallets '
           'produced using version 0.5.X and earlier.  '
           'You can determine the version by '
           'opening the regular Terracoin client, then choosing "Help"'
           '-->"About Terracoin-Qt" from the main menu.  '
           '<br><br>'
           '<b>If you have used your wallet with any version of the regular '
           'Terracoin client 0.6.0 or higher, this tool <u>will fail</u></b>.  '
           ...
           'by Armory.'
           '<br><br>'
           'Has your wallet ever been opened in the 0.6.0+ Terracoin-Qt client?', \
   ...
   def clickSendBitcoins(self):
      ...
      if len(self.walletMap)==0:
         reply = QMessageBox.information(self, 'No Wallets!', \
            'You cannot send any terracoins until you create a wallet and '
   ...
   def uriSendBitcoins(self, uriDict):
      # Because Terracoin-Qt doesn't store the message= field we have to assume
      ...
      descrStr = ''
      descrStr = ('You just clicked on a "terracoin:" link requesting terracoins ' 
      ...
      if uri_has('amount'):
         ...
         descrStr += '<br>--<b>Amount</b>:\t%s TRC' % amtstr
      ...
      if len(self.walletMap)==0:
         reply = QMessageBox.information(self, 'No Wallets!', \
            'You just clicked on a "terracoin:" link to send money, but you '
   ...
   def clickReceiveCoins(self):
      LOGDEBUG('Clicked "Receive Terracoins Button"')
      wltID = None
      selectionMade = True
      if len(self.walletMap)==0:
         reply = QMessageBox.information(self, 'No Wallets!', \
            'You have not created any wallets which means there is nowhere to '
            'store you terracoins!  Would you like to create a wallet now?', \
            QMessageBox.Yes | QMessageBox.No)
         if reply==QMessageBox.Yes:
            self.createNewWallet(initLabel='Primary Wallet')
         return
   ...
   def setDashboardDetails(self):
      ...
      txtScanFunc = ( \
         ...
         '<li>Sign messages</li>'
         '</ul>'
         '<br><br><b>NOTE:</b>  The Terracoin network <u>will</u> process transactions '
      ...
      txtOnlineFunc = ( \
         ...
         '<li>Send terracoins to other people</li>'
         '<li>Create one-time backups of your wallets (in printed or digital form)</li>'
         '<li>Click on "terracoin:" links in your web browser '
      ...
      if TheBDM.getBDMState() in ('Offline', 'Uninitialized'):
         if onlineAvail and not self.lastBDMState[1]==onlineAvail:
            ...
            self.lblDashDescr.setText('You are currently in offline mode, but can '
               ...
               'Terracoin-Qt/terracoind is fully synchronized with the terracoin network.  '
               'You will see a green checkmark in the bottom-right corner of '
               'the Terracoin-Qt window when it is finished.'
               '<br><br>'
               'Switching to online mode will give you access '
               'to more Armory functionality, including sending and receiving '
               'terracoins and viewing the balances and transaction histories '
            ...
            if not self.bitcoindIsAvailable():
               if self.internetAvail:
                  ...
                  lblText = ('You are currently in offline mode because '
                             'Terracoin-Qt is not running.  To switch to online ' 
                             'mode, start Terracoin-Qt and let it synchronize with the network '
                  ...
                  lblText = ('No internet connection was detected, and neither '
                             'Terracoin-Qt or terracoind is running.  Most likely '
                             ...
                             'start Terracoin-Qt and let it synchronize with the '
                             'network (a green checkmark will appear in the bottom '
                             'right corner of the Terracoin-Qt window when it is '
                             'finished).  Then restart Armory.'
                             '<br><br>'
                             'If you do not have Terracoin-Qt installed, you can '
                             'download it from <a href="http://www.terracoin.org">'
                             'http://www.terracoin.org</a>.')
               ...
               lblText = ('You are currently in offline mode because '
                          ...
                          'that it is not necessary to have Terracoin-Qt or terracoind '
                          'running.' )
               ...
               lblText = ('You are currently in offline mode because '
                          ...
                          'by Terracoin-Qt.  Do you run Terracoin-Qt (or terracoind) '
         ...
         if self.netMode == NETWORKMODE.Disconnected:
            ...
            self.lblDashDescr.setText( \
               'Armory was previously online, but the connection to Terracoin-Qt/'
               'terracoind was interrupted.  You will not be able to send terracoins or '
               'confirm receipt of terracoins until the connection is reestablished.  '
               '<br><br>Please check that Terracoin-Qt is open '
               'and synchronized with the network.  Armory will <i>try to reconnect</i> '
               'automatically when the connection is available again.  If Terracoin-Qt is '
            ...
            self.lblDashDescr.setText( \
               ...
               'Here\'s some things you can do with Armory Terracoin Client:'
               '<br>' + txtOnlineFunc + '<br>'
               'If you experience any performance issues with Armory, '
               'please confirm that Terracoin-Qt is running and <i>fully '
               'synchronized with the Terracoin network</i>.  You will see '
               'a green checkmark in the bottom right corner of the '
               'Terracoin-Qt window if it is synchronized.  If not, it is '
   ...
   def Heartbeat(self, nextBeatSec=1):
      ...
      try:
         ...
         if TheBDM.getBDMState() in ('Offline','Uninitialized'):
            ...
            # mode, it needs to check periodically for the existence of Terracoin-Qt
         ...
         if TheBDM.getBDMState()=='BlockchainReady':
            ...
            if( blksInLast5sec>10 ):
               LOGERROR('Detected Terracoin-Qt/terracoind not synchronized')
               LOGERROR('New blocks added in last 5 sec: %d', blksInLast5sec)
               if self.noSyncWarnYet:
                  self.noSyncWarnYet = False
                  QMessageBox.warning(self,'Terracoin-Qt is not synchronized', \
                     'Armory has detected that Terracoin-Qt is not synchronized '
                     'with the terracoin network yet, and Armory <b>may</b> not '
                     ...
                     'corner of the Terracoin-Qt window.', QMessageBox.Ok)
               return
   ...
   def doTheSystemTrayThing(self):
      ...
      for i in range(len(self.notifyQueue)):
         ...
         if le.isSentToSelf():
            amt = self.determineSentToSelfAmt(le, wlt)[0]
            self.sysTray.showMessage('Your terracoins just did a lap!', \
               'Wallet "%s" (%s) just sent %s TRC to itself!' % \
            ...
            if le.getValue()>0:
               # Received!
               title = 'Terracoins Received!'
               totalStr = coin2str( sum([mine[i][1] for i in range(len(mine))]), maxZeros=1)
               dispLines.append(   'Amount: \t%s TRC' % totalStr.strip())
               if len(mine)==1:
